...
`):"No printer breakdown available"}function mn({loc:t,navigate:e}){return j`<nav class="top-nav">
  ...
  `}function Sn({job:t,onJobClick:e}){return j`
  ...
  `}function Pn({sorted:t,onJobClick:e,density:n}){return j`
    <div class=${"grid-view density-"+n}>
      ${t.map(a=>j`<${Sn} key=${a.id} job=${a} onJobClick=${e} />`)}
    </div>
    ...
    <main class="catalog-page">
      <section class="admin-section">
        <div class="catalog-header-row">
          ...
          <button class="btn-primary" onClick=${async()=>{c(!0);try{const u=await dt("/catalog/scan",{},"Catalog scan failed.");if(!u)return;i(u.summary),Y("Catalog scan complete.",u.summary.failed>0?"info":"success"),await p()}finally{c(!1)}}} disabled=${d||t.every(u=>!u.is_active)}>
            ${d?"Scanning…":"Run scan"}
          </button>
        </div>
        <${oa} summary=${_} />
  ...
  />`}function ua({sorted:t,view:e,sortCol:n,sortDir:a,onSort:r,onJobClick:s,density:o}){return t.length===0?D`<div class="empty">No jobs match your filters.</div>`:e==="table"?D`<${kn}
      ...
      onJobClick=${s}
      density=${o}
    />`:D`<${Pn} sorted=${t} onJobClick=${s} density=${o} />`}function pa({q:t,setQ:e,statusFilter:n,setStatusFilter:a,deviceFilter:r,setDeviceFilter:s,devices:o,view:l,setView:d,filtered:c,jobs:_,isFiltered:i,sorted:p,sortCol:v,sortDir:m,onSort:b,onJobClick:u,density:f,setDensity:$}){return D`
    <${bn}
      q=${t}
      setQ=${e}
      statusFilter=${n}
