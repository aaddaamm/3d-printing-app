...
const { mockUpsertMaterialRun, mockGetMaterial, mockUpsertMachineRun, mockGetMachine } = vi.hoisted(
  () => ({
    mockUpsertMaterialRun: vi.fn(),
    mockGetMaterial: vi.fn(),
    mockUpsertMachineRun: vi.fn(),
    mockGetMachine: vi.fn(),
  }),
);

vi.mock("../lib/db.js", () => ({
  stmts: {
    upsertMaterialRate: { run: mockUpsertMaterialRun },
    ...
    upsertMachineRate: { run: mockUpsertMachineRun },
    getMachineRate: { get: mockGetMachine },
  },
}));

import { upsertMachineRate, upsertMaterialRate } from "../models/rates.js";
...
describe("upsertMaterialRate", () => {
  ...
  it("computes rate_per_g as cost_per_g * (1 + waste_buffer_pct)", () => {
    ...
    const { rate_per_g } = mockUpsertMaterialRun.mock.calls[0]![0] as { rate_per_g: number };
  ...
  it("regression: does NOT divide waste_buffer_pct by 100 (fraction not percent)", () => {
    ...
    expect(rate_per_g).toBeGreaterThan(0.03);
  ...
  it("passes all input fields through to the DB unchanged", () => {
    ...
    const call = mockUpsertMaterialRun.mock.calls[0]![0] as Record<string, unknown>;
...
describe("upsertMachineRate", () => {
  ...
  it("computes machine_rate_per_hr from depreciation, electricity, and maintenance", () => {
    upsertMachineRate({
      device_model: "X1C",
      purchase_price: 1000,
      lifetime_hrs: 5000,
    ...
    const { machine_rate_per_hr } = mockUpsertMachineRun.mock.calls[0]![0] as {
  ...
  it("rejects zero lifetime_hrs before writing to the DB", () => {
    ...
    ).toThrow("lifetime_hrs must be greater than 0");
    expect(mockUpsertMachineRun).not.toHaveBeenCalled();
  });
});
